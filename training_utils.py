...
def materialize_yolo_class_mapping(
    ...
    *,
    ...
    """Create a YOLO dataset copy with class IDs remapped by class name.

    Images are hard-linked when possible and copied as a fallback. Label files are
    rewritten because YOLO stores class IDs, not class names, in each annotation row.
    """
    import yaml
    ...
    if not source_dataset_dir.exists():
        raise FileNotFoundError(f"Source dataset directory was not found: {source_dataset_dir}")
    if source_dataset_dir.resolve() == output_dataset_dir.resolve():
        raise ValueError("Output dataset directory must be different from the source dataset directory.")

    target_id_by_name = {name: index for index, name in enumerate(target_class_names)}
    old_id_to_new_id: dict[int, int] = {}
    for old_id, old_name in enumerate(original_class_names):
        mapped_name = class_name_mapping.get(old_name, old_name)
        if mapped_name not in target_id_by_name:
            raise ValueError(
                f"Class '{old_name}' maps to '{mapped_name}', which is missing from target_class_names."
            )
        old_id_to_new_id[old_id] = target_id_by_name[mapped_name]
    ...
    if output_dataset_dir.exists() and overwrite:
        ...
        if marker_path.exists() or output_dataset_dir.name == expected_generated_name:
            ...
            raise FileExistsError(
                f"Refusing to overwrite {output_dataset_dir} because it was not generated by this helper."
            )

    output_dataset_dir.mkdir(parents=True, exist_ok=True)

    split_names = ["train"]
    split_names.append("val" if (source_dataset_dir / "val").exists() else "valid")
    ...
    for split in split_names:
        ...
        output_split_dir = output_dataset_dir / split
        output_images_dir = output_split_dir / "images"
        output_labels_dir = output_split_dir / "labels"
        output_images_dir.mkdir(parents=True, exist_ok=True)
        output_labels_dir.mkdir(parents=True, exist_ok=True)

        for image_path in _list_images(source_split_dir / "images"):
            _copy_or_link_file(image_path, output_images_dir / image_path.name)
            copied_images += 1
        ...
        for label_path in sorted(source_labels_dir.glob("*.txt")):
            remapped_lines = []
            for line_number, line in enumerate(label_path.read_text(encoding="utf-8").splitlines(), start=1):
                ...
                try:
                    ...
                    raise ValueError(f"{label_path} line {line_number}: invalid class id '{parts[0]}'.") from exc
                if old_id not in old_id_to_new_id:
                    raise ValueError(f"{label_path} line {line_number}: class id {old_id} is not in original_class_names.")
    ...
    yaml_payload = {
        ...
        "train": "train/images",
        "val": f"{'val' if (output_dataset_dir / 'val').exists() else 'valid'}/images",
        "test": "test/images",
        "nc": len(target_class_names),
        "names": target_class_names,
    }
    (output_dataset_dir / "data.yaml").write_text(yaml.safe_dump(yaml_payload, sort_keys=False), encoding="utf-8")
    ...
    marker_payload = {
        ...
        "copied_or_linked_images": copied_images,
        "rewritten_label_rows": rewritten_rows,
    }
    (output_dataset_dir / ".generated_class_mapping.json").write_text(
